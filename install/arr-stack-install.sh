#!/usr/bin/env bash
# Runs INSIDE the Arr Stack LXC. Do not execute on the Proxmox host.
set -eEuo pipefail
...
latest_asset() {
  ...
  curl -fsSL -H 'Accept: application/vnd.github+json' "https://api.github.com/repos/${repo}/releases/latest" \
    | jq -r --arg arch "$ASSET_ARCH" --arg prefix "$prefix" \
      '.assets[] | select((.name | startswith($prefix)) and (.name | test("(^|\.)linux(-musl)?(-core)?-" + $arch + "\.tar\.gz$"))) | .browser_download_url' | head -n1
}

install_arr() {
  local app="$1" repo="$2" service="$3" url tmp
  url="$(latest_asset "$repo" "$app")"
  [[ -n "$url" && "$url" != null ]] || { echo "No compatible ${app} release asset found" >&2; exit 1; }
  log "Installing ${app}"
...
install_seerr() {
  log "Installing Node.js and Seerr"
...
enable_console_autologin() {
  # Proxmox web console normally attaches to tty1. This enables root autologin there.
  # pct enter remains the safer management method and does not require a password.
  log "Enabling tty1 root autologin"
